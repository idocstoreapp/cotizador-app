              ...
              >
                <div className="aspect-square bg-gray-100">
                  <img
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = 'https://via.placeholder.com/400x400?text=' + encodeURIComponent(closet.nombre);
                    }}
                  />
                </div>
                <div className="p-4">
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">{closet.nombre}</h3>
                  {closet.descripcion && (
                    <p className="text-sm text-gray-600 mb-3 line-clamp-2">{closet.descripcion}</p>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="text-xl font-bold text-indigo-600">
                      ${closet.precio_base.toLocaleString('es-CO')}
                    </span>
